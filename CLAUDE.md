...
12. **`scope upgrade pending approval` from in-container CLI** → If a `railway ssh` invocation (or the agent's exec tool) hits `gateway closed (1008): pairing required: device is asking for more scopes than currently approved`, the wrapper's auto-approval loop probably can't see the request. Two root causes:
    ...
    - **Auto-approval cadence**: the loop polls every 3-60s. A fresh upgrade request may take one tick to clear. If urgent, run `openclaw devices clear --yes` to reset; the next CLI invocation will pair fresh and the loop catches it. Permanent fix is the deviceAuth.js patch above; manual `clear` was the workaround pre-fix.
