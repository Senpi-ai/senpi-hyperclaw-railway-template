...
11. **Senpi runtime plugin — DEV channel + @beta dist-tag on this branch.** `integration/openclaw-v2026.5.7-full` installs the plugin from **`@senpi/runtime@beta`** (dev npm channel, prerelease dist-tag) — NOT `@senpi-ai/runtime` (prod). The dev channel publishes from the `openclaw-upgrade/main` branch of `senpi-trading-runtime` (CI: dist-tag = `beta`) and carries the OpenClaw v2026.5.x compatibility fixes the prod 1.1.0 line is missing:
