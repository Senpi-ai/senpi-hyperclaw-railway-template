...
11. **Senpi runtime plugin — DEV channel on this branch.** `integration/openclaw-v2026.5.7-full` installs the plugin from **`@senpi/runtime`** (dev npm channel) rather than `@senpi-ai/runtime` (prod). The dev channel publishes from the `openclaw-upgrade/main` branch of `senpi-trading-runtime` and currently carries two fixes the prod 1.1.0 line is missing: `resolvePackageName()` so auto-update polls the same channel it was installed from, and `commandAliases` + `activation.onCommands` in `openclaw.plugin.json` so `openclaw senpi …` CLI dispatch works on OpenClaw v2026.5.x. Both fixes are in upstream PRs (resolvePackageName: #99); cutover to `@senpi-ai/runtime` happens when those merge + a 1.2.0 prod release ships. Until then, deployments from this branch pull `@senpi/runtime@latest` on every boot. In `openclaw.json`, `plugins.allow` / `plugins.entries` still use the **manifest id** `runtime` (OpenClaw derives `idHint` from the unscoped npm name regardless of which scope). Set `SENPI_TRADING_RUNTIME_ENABLED=false` to disable.
