...
11. **Senpi runtime plugin — canonical PROD scope `@senpi-ai/runtime`, env-driven spec/name/nonce (reverted 2026-07-06, fleet-update Workstream A).** `bootstrap.mjs` no longer hardcodes a dev spec. `SENPI_RUNTIME_NPM_SPEC` (default `@senpi-ai/runtime`) is what `openclaw plugins install` receives and `SENPI_RUNTIME_NPM_NAME` (default `@senpi-ai/runtime`) is the bare name the managed install-path probe embeds — keep both in lock-step when overriding to a dev `@senpi/…` build, or the probe looks under the wrong scope. On spec change **or** a changed `SENPI_RUNTIME_REINSTALL_NONCE` (absent/empty = never force; one-shot latch recorded in `cfg.plugins.installs["runtime"]`), bootstrap wipes the ENTIRE `STATE_DIR/npm/node_modules` tree (not just the plugin dir) so hoisted transitive deps re-resolve/downgrade, then reinstalls and emits `SENPI_RUNTIME_INSTALLED spec=… version=… nonce=…`. Wipe scope is asserted safe: `senpi-state/` strategies, `agents/` sessions, workspace, and device pairing are all siblings, never under `npm/`. The pure trigger logic lives in `src/lib/runtimeInstallDecision.mjs` (unit-tested). **Historical note (pre-revert):** `integration/openclaw-v2026.5.7-full` had temporarily installed from the DEV channel (`@senpi/runtime@beta`, later a throwaway `branch-test-spans-…` build) while OpenClaw v2026.5.x compatibility fixes hadn't reached prod. Those fixes carried by the dev channel were:
