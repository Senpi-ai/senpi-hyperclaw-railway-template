...
8. **"pairing required" / "connect failed" (1008)** → If logs show `[telegram] starting provider` then `gateway connect failed: Error: pairing required`, the **Telegram provider** (internal client) is the one being rejected, not the browser. Wrapper logs `[ws-upgrade] Proxying WebSocket to gateway` only for browser→wrapper→gateway; absence of that line at the same timestamp means the failing connection is internal (Telegram plugin→gateway). We set `allowInsecureAuth`, `trustedProxies`, and inject the token for proxy traffic; internal clients must use the token from config (Openclaw upstream). If it persists, try a different Openclaw build or see openclaw/openclaw#4941.
