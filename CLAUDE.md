...
6. **WebSocket auth requires proxy event handlers** → Direct `req.headers` modification doesn't work for WebSocket upgrades with http-proxy; the wrapper uses the `proxyReqWs` event in `src/routes/proxy.js` to inject the `Authorization` header before forwarding.
7. **Control UI** → `gateway.controlUi.allowInsecureAuth=true` is always set (Control UI sits behind the wrapper's Basic-Auth proxy). `gateway.controlUi.dangerouslyDisableDeviceAuth` is opt-in via `OPENCLAW_DANGEROUSLY_DISABLE_DEVICE_AUTH=true` (default OFF); see `src/lib/dangerousAuthFlag.js` for the rationale. The four write-sites (`bootstrap.mjs`, `onboard.js`, `gateway.js`, `setup.js`) must all stay in lock-step on these knobs — use the helpers in `src/lib/`.
8. **"pairing required" / "connect failed" (1008) from an external bridge** → The orchestrator-coordinated pairing flow (`/setup/api/issue-bootstrap-token` + `/setup/api/approve-device-pairing`) is the single approval path. If a bridge connect stalls, check that the orchestrator's `/v1/agents/me/repair` was called and that `AGENT_BRIDGE_ORIGIN` matches across wrapper, orchestrator, and bridge.
