...
14. **External agent-bridge integration (`/openclaw/ws` + auto-approval)** → The wrapper exposes a dedicated WS path (`AGENT_BRIDGE_WS_PATH`, default `/openclaw/ws`) that bypasses the `SETUP_PASSWORD` Basic-auth gate AND skips the wrapper's bearer-token injection. The bridge (Senpi-ai/agent-bridge, `v3/go-rewrite`) authenticates via `auth.token` inside its v3 `connect` request (`internal/openclaw/handshake.go`), not via HTTP headers.
    ...
    **Pairing**: the bridge sends `client.id=webchat-ui mode=webchat role=operator scopes=[chat]`. (Role MUST be `operator` — OpenClaw v2026.5.x's `GatewayRole` enum is `["operator", "node"]` in `src/gateway/role-policy.ts:3`; `user` is rejected with `INVALID_REQUEST`.) OpenClaw places this in `pendingRequests` after validating the bootstrap token against the gateway shared secret. The wrapper's auto-approval loop (`src/lib/deviceAuth.js`) matches it against the `isAgentBridgeRequest` allowlist and approves via the local Node-import path with `callerScopes:["operator.admin"]`.
