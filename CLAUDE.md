...
11. **Senpi runtime plugin — canonical PROD scope `@senpi-ai/runtime`, env-driven spec/name/nonce (reverted 2026-07-06, fleet-update Workstream A).** `bootstrap.mjs` no longer hardcodes a dev spec. `SENPI_RUNTIME_NPM_SPEC` (default `@senpi-ai/runtime`) is what `openclaw plugins install` receives and `SENPI_RUNTIME_NPM_NAME` (default `@senpi-ai/runtime`) is the bare name the managed install-path probe embeds — keep both in lock-step when overriding to a dev `@senpi/…` build, or the probe looks under the wrong scope. On spec change, a changed `SENPI_RUNTIME_REINSTALL_NONCE` (absent/empty = never force; one-shot latch recorded in `cfg.plugins.installs["runtime"]`), **or a missing install record** (dirty volumes that lost their record still carry a stale tree; free no-op on genuinely fresh volumes), bootstrap wipes the managed npm root artifacts under `STATE_DIR/npm` — `node_modules/`, `package-lock.json`, AND `package.json` (`NPM_WIPE_TARGETS`) — so hoisted transitive deps re-resolve/downgrade, then reinstalls and emits `SENPI_RUNTIME_INSTALLED spec=… version=… nonce=…`. Wiping `node_modules` alone is NOT enough: `openclaw plugins install` runs plain `npm install` in `STATE_DIR/npm` with the lockfile honored (openclaw v2026.5.7 `src/plugins/install.ts:1375-1398` passes `packageLock: true` → `npm_config_package_lock=true`), so a surviving `package-lock.json` re-pins the exact stale tree; and the managed `package.json` merges dependencies on upsert (`src/infra/npm-managed-root.ts:184-224`), so a stale old-scope entry would be reinstalled alongside the new one. All three are regenerated by openclaw on the next install. Wipe scope is asserted safe: `senpi-state/` strategies, `agents/` sessions, workspace, and device pairing are all siblings, never under `npm/`. The pure trigger logic + wipe-target set live in `src/lib/runtimeInstallDecision.mjs` (unit-tested). **Historical note (pre-revert):** `integration/openclaw-v2026.5.7-full` had temporarily installed from the DEV channel (`@senpi/runtime@beta`, later a throwaway `branch-test-spans-…` build) while OpenClaw v2026.5.x compatibility fixes hadn't reached prod. Those fixes carried by the dev channel were:
    - `resolvePackageName()` — auto-update polls the same channel it was installed from (PR #99).
    - Plugin export id `"runtime"` matches `openclaw.plugin.json` (v2026.5.x strict-rejects mismatches; v2026.2.x was lenient).
    - `commandAliases` + `activation.onCommands` in `openclaw.plugin.json` — `openclaw senpi …` CLI dispatch works.
    - `--dangerously-force-unsafe-install` retired via `utils/safe-spawn.ts` refactor (see quirk #13).

    Those fixes have since merged to prod, hence the revert to `@senpi-ai/runtime`. In `openclaw.json`, `plugins.allow` / `plugins.entries` still use the **manifest id** `runtime` (OpenClaw derives `idHint` from the unscoped npm name regardless of which scope). Set `SENPI_TRADING_RUNTIME_ENABLED=false` to disable.

12. **`scope upgrade pending approval` from in-container CLI** → If a `railway ssh` invocation (or the agent's exec tool) hits `gateway closed (1008): pairing required: device is asking for more scopes than currently approved`, the wrapper's auto-approval loop probably can't see — or can't approve — the request. Three root causes, all fixed on this branch:
    - **v2026.5.x JSON shape**: `openclaw devices list --json` now returns `{ pending: [...], paired: [...] }` not a flat array. Fixed in `src/lib/deviceAuth.js` via `extractPendingRequests` (handles both shapes) — see commit `f4c3273`. If you see the symptom on this branch, it's something else.
    - **v2026.5.x scope-escalation in CLI approve subprocess**: `openclaw devices approve <reqId>` runs as the wrapper's auto-paired CLI device, which only holds `operator.pairing`. v2026.5.x's `resolveApprovePairingScopesForRequest` (`src/cli/devices-cli.ts:290`) refuses approvals whose target scopes exceed the caller's. The built-in local-file fallback in openclaw only triggers on "pairing required" — not on "scope upgrade pending approval" — so the wrapper subprocess dead-loops on managed-agent boxes with no human approver. **Fixed** in `src/lib/devicePairingNode.js`: the wrapper now imports openclaw's `approveDevicePairing()` directly from `openclaw/plugin-sdk/device-bootstrap` and calls it with `callerScopes: ["operator.admin"]` (the documented local-trust escape hatch). The CLI subprocess remains as a fallback if the Node import path is unavailable (misconfigured `OPENCLAW_ENTRY`, bundle-layout change). Companion change in the runtime plugin: PR Senpi-ai/senpi-trading-runtime#102 declares per-method `OperatorScope` on every `senpi.*` gateway method so they no longer default to `operator.admin`.
    - **Auto-approval cadence**: the loop polls every 3-60s. A fresh upgrade request may take one tick to clear. If urgent, run `openclaw devices clear --yes` to reset; the next CLI invocation will pair fresh and the loop catches it. Permanent fix is the deviceAuth.js + devicePairingNode.js patches above; manual `clear` was the workaround pre-fix.

13. **`--dangerously-force-unsafe-install` retired from `openclaw plugins install`** → OpenClaw v2026.5.x's install gate (skill-scanner.ts dangerous-exec rule) flags any plugin file with both a `spawn(/exec(` call and the literal `child_process` in the same file. The wrapper used to pass `--dangerously-force-unsafe-install` so the trusted, first-party senpi plugin could install through. As of `@senpi/runtime` `1.2.0-dev.openclaw-upgrade-mai.<timestamp>` (after the safe-spawn refactor — see plugin commit `89a07f7`), the compiled plugin JS no longer contains the literal, so the gate doesn't fire and the bypass flag is gone. **If `openclaw plugins install` ever fails with "dangerous code patterns detected"**, it means a future plugin publish reintroduced the literal — check `dist/runtime/auto-update/command-runner.js`, `dist/cli/senpi-commands.js`, and `dist/utils/safe-spawn.js`. The plugin's `src/utils/__tests__/safe-spawn.test.ts` has a regression-guard test that fails if `dist/utils/safe-spawn.js` ever contains `child_process`.

14. **External agent-bridge integration (`/openclaw/ws` + auto-approval)** → The wrapper exposes a dedicated WS path (`AGENT_BRIDGE_WS_PATH`, default `/openclaw/ws`) that bypasses the `SETUP_PASSWORD` Basic-auth gate AND skips the wrapper's bearer-token injection. The bridge (Senpi-ai/agent-bridge, `v3/go-rewrite`) authenticates via `auth.token` inside its v3 `connect` request (`internal/openclaw/handshake.go`), not via HTTP headers.

    **Pairing**: the bridge sends `client.id=webchat-ui mode=webchat role=operator scopes=[operator.admin, operator.approvals]`. (Role MUST be `operator` — OpenClaw v2026.5.x's `GatewayRole` enum is `["operator", "node"]` in `src/gateway/role-policy.ts:3`; `user` is rejected with `INVALID_REQUEST`. Scopes MUST be from `src/gateway/operator-scopes.ts` — `chat` is not valid. The bridge needs `operator.admin` for chat.send/sessions.* and the **separate** `operator.approvals` for `exec.approval.resolve`/`plugin.approval.resolve`; admin does NOT auto-imply approvals per `src/shared/device-auth.ts`.) OpenClaw places this in `pendingRequests` after validating the bootstrap token against the gateway shared secret. The wrapper's auto-approval loop (`src/lib/deviceAuth.js`) matches it against the `isAgentBridgeRequest` allowlist (default now `["operator.admin", "operator.approvals"]`, was the broken `["chat"]`) and approves via the local Node-import path with `callerScopes:["operator.admin", "operator.approvals"]` so both requested scopes can be granted to the device.

    **Trust boundary**: `client.id` is metadata the connecting peer types — not a security gate. An attacker with the bootstrap token can already claim any allowlisted id; restricting the allowlist to one id gives no extra defense. The allowlist exists to narrow the auto-approval to the chat path so operator-scope upgrades still require a human approver. The **real** gate is OpenClaw's bootstrap-token validation upstream (`handshake-auth-helpers.ts`). Document the threat: leaking `OPENCLAW_BOOTSTRAP_TOKEN` = full chat-scope access on that deployment.

    **Operator UX**: `GET /setup/api/agent-bridge-creds` (Basic-auth-gated) returns `{gatewayUrl, bootstrapToken, agentId}` — the three env vars the bridge needs (`OPENCLAW_GATEWAY_URL`, `OPENCLAW_BOOTSTRAP_TOKEN`, `OPENCLAW_AGENT_ID`). Each read is audit-logged.
